...
def train_mle_estimator(train_images, train_labels):
    ...
    # N = number of images, D represents 1-of-10 encoded class
    sum_labels = np.sum(train_labels, axis=0)
    pi_mle = sum_labels/train_labels.shape[0]
    theta_mle = np.dot(train_images.T, train_labels)/sum_labels
...
def train_map_estimator(train_images, train_labels):
    ...
    pi_map = labels_sum/train_images.shape[0]
    theta_map = np.divide(np.dot(train_images.T, train_labels) + 2, np.sum(train_labels, axis=0) + 4)
...
def log_likelihood(images, theta, pi):
    ...
    second = logsumexp(first, axis=1)
    log_like = (first.T - second).T
    return log_like
...
def predict(log_like):
    ...
    # x that maximizes log likelihood
...
def accuracy(log_like, labels):
    ...
    accuracy = np.mean(prediction == np.argmax(labels, axis=1))
    return accuracy
...
def image_sampler(theta, pi, num_images):
    ...
    for i in range(num_images):
        sampled_images.append([np.random.binomial(n=1, p=theta.T[c[i]][j]) for j in range(784)])
